      ...
      for i = 1, table.getn(moves) do
        for j = 1, moves[i] do
          ...
          local newEnemy =
            EnemyLeftRight:new(
            j % 2,
            (side - 1) * (screenWidth - 77) / 3,
            side * (screenWidth - 77) / 3,
            down,
            down + 150
          )
          newEnemy:setLevel(level)
    ...
    function class:setLevel(level)
      ...
      elseif (moveType == 3) then
        ...
        for i = 1, numEnemies do
          ...
          if (i % 2 == 1) then
            newEnemy.x =
              (math.floor(numEnemies / 2) + math.floor(i / 2) + 1 - 1) * (screenWidth + newEnemy.width) /
              (numEnemies + 1) +
              newEnemy.width
          else
            newEnemy.x =
              (math.ceil(numEnemies / 2) - (math.floor((i - 1) / 2) + 1) - 1) * (screenWidth + newEnemy.width) /
              (numEnemies + 1) +
              newEnemy.width
          end
          table.insert(self.enemies, newEnemy)
