...
function Enemies.init()
  ...
  Enemies.enemyImgs = {
    gfx.newImage("assets/enemies/enemy_1.png"),
  ...
  Enemies.asteroidImgs = {
    gfx.newImage("assets/asteroids/asteroid_L_1.png"),
...
function Enemies.enemyHit(enemy, index)
  ...
  if enemy.hitCounter <= 0 then
    ...
    if (Sounds.explodeSound:isPlaying()) then
      Sounds.explodeSound:seek(0, "seconds")
      --Sounds.explodeSound:rewind() --deprecated in lua v11
    else
      Sounds.explodeSound:play()
    end
    ...
    if not enemy.isBoss then
      enemy.hitClock = 0.2
      if (Sounds.explodeSound:isPlaying()) then
        Sounds.explodeSound:seek(0, "seconds") -- rewind
        --Sounds.explodeSound:rewind() --deprecated in lua v11
      else
        Sounds.explodeSound:play()
      end
