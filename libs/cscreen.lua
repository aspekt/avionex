...
function CScreen.getData()
  return {tx/tc, ty/tc, fsv}
end

-- Scales and centers all graphics properly
function CScreen.apply()
	if ctr then
		love.graphics.translate(tx/tc, ty/tc)
	end
	love.graphics.scale(fsv, fsv)
end

function CScreen.adjustTx(aTx)
  tx = tx - aTx
end
